...
}); //смотреть, чтоб в main.js не было пробелов после линков
...
gulp.task('style:build', function () {
    ...
    .pipe(plumber())
        ...
        .pipe(prefixer({ browsers: ['last 2 versions',   "Android 2.3",
  "Android >= 4", "Chrome >= 20", "Firefox >= 24", "Explorer >= 8", "iOS >= 6", "Opera >= 12", "Safari >= 6"] })) //Добавим вендорные префиксы
        .pipe(cssmin()) //Сожмем
...
}); //найдет те файлы, где были изменения и запустит для них нужную задачу
    ...
    return gulp.src(["src/icons/*.svg"]) //path to svg icons
      ...
      .on("glyphs", (glyphs) => {
        gulp.src("src/icons/util/*.scss") // Template for scss files
            ...
            .pipe(gulp.dest("src/style/partials/")); // generated scss files with classes
      })
...
gulp.task('sprites:build', function () {
    var spriteData =
        gulp.src('src/img/sprites/*.png') //выберем откуда брать изображения для объединения в спрайт
            .pipe(spritesmith({
                imgName: 'sprite.png', //имя спрайтового изображения
                cssName: '_sprite.scss', //имя стиля где храним позиции изображений в спрайте
                imgPath: 'src/img/sprites/sprite.png', //путь где лежит спрайт
    ...
    spriteData.css.pipe(gulp.dest("src/style/partials/sprites/")); // путь, куда сохраняем стили
}); //спрайты
